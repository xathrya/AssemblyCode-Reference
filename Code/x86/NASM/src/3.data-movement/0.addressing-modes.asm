...
;---------------------------------------------------
; In most two operands instructions, one operand acts
; as source operand and the another one acts as 
; destination operand.
;
...
;---------------------------------------------------
; Immediate operands are the immediate values
...
;
; Index is the i-th chunk of data, with size of `scale`.
; It can be a general purpose registers:
;   EAX, EBX, ECX, EDX, EBP, ESI, EDI
;   R8, R9, R10, R11, R12, R13, R14, R15
;
; Displacement is a direct (uncomputed) offset to the address.
...
; addressing a location.
    ...
    mov     r8d, [WORD_TABLE + eax + 4]
    mov     [WORD_TABLE + eax + 4], r9d

    ; [6] Base + Index * Scale + Displacement
    ; Using all addressing components for accessing address
    mov     r10d, [WORD_TABLE + eax * 4 + 4]
    mov     [WORD_TABLE + eax * 4 + 4], r10d
